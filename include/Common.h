...
inline std::pair<T, T> modDiv(T, T) {
...
static inline constexpr bool isInChunkBounds(const Coordinate3D& coord) {
	return coord.x < Constants::chunkWidth 
		&& coord.y < Constants::chunkHeight 
		&& coord.z < Constants::chunkDepth;
}
...
namespace Id
{
	...
	struct alignas(4) IndexSequenceCacheEntry {
		static_assert(std::is_trivial_v<Metadata> && std::is_standard_layout_v<Metadata> && alignof(Metadata) <= 4);
		uint32_t start;
		uint32_t size;

		Metadata metadata;

		bool operator==(const IndexSequenceCacheEntry& other) const {
			return start == other.start && size == other.size && metadata == other.metadata;
		};
	};

	template<>
	struct alignas(4) IndexSequenceCacheEntry<void> {
		uint32_t start;
		uint32_t size;
	...
	template<typename Index, typename EntryT>
	class IndexSequenceCacheBase
	{
	public:
		struct EntryTag {};
	...
	template<typename Index, typename Comparator = std::equal_to<Index>, typename Metadata = void>
	class IndexSequenceCache : public IndexSequenceCacheBase<Index, IndexSequenceCacheEntry<Metadata>>
	{
	public:
		using Base = IndexSequenceCacheBase<Index, IndexSequenceCacheEntry<Metadata>>;
		using EntryId = typename Base::EntryId;
		using Entry = typename Base::Entry;
		using EntryCache = typename Base::EntryCache;
	public:
		EntryId add(const std::vector<Index>& sequence, Metadata metadata)
		{
			if (sequence.empty()) return this->m_entries.add(Entry{ 0, 0, metadata });
			Entry entry;
			...
			{
				if (Comparator()(this->m_indices[i], sequence[0]))
				{
					size_t j = i + 1;
					size_t k = 1;
					for (; j < this->m_indices.size() &&
						k < sequence.size() &&
						Comparator()(this->m_indices[j], sequence[k]);
						++j, ++k);
					if (k == sequence.size())
					{
						entry.start = i;						
			...
			entry.start = this->m_indices.size();
			this->m_indices.insert(this->m_indices.end(), sequence.begin(), sequence.end());
			return this->m_entries.add(entry);
		}
	};

	template<typename Index, typename Comparator>
	class IndexSequenceCache<Index, Comparator, void> : public IndexSequenceCacheBase<Index, IndexSequenceCacheEntry<void>>
	{
	public:
		using Base = IndexSequenceCacheBase<Index, IndexSequenceCacheEntry<void>>;
		using EntryId = typename Base::EntryId;
		using Entry = typename Base::Entry;
		using EntryCache = typename Base::EntryCache;
	public:
		...
		{
			if (sequence.empty()) return this->m_entries.add(Entry{ 0, 0 });
			Entry entry;
			entry.size = sequence.size();
